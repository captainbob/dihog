...
[![NPM version](https://img.shields.io/npm/v/dihog.svg?style=flat)](https://npmjs.org/package/dihog)
[![Build Status](https://img.shields.io/travis/sorrycc/dihog.svg?style=flat)](https://travis-ci.org/sorrycc/dihog)
[![NPM downloads](http://img.shields.io/npm/dm/dihog.svg?style=flat)](https://npmjs.org/package/dihog)
[![Dependencies](https://david-dm.org/sorrycc/dihog/status.svg)](https://david-dm.org/sorrycc/dihog)

[View README in English](./README_en-us.md)

dihog 是一个 cli 工具，提供 `server`、 `build` 和 `test` 三个命令，分别用于本地调试和构建，并且提供了特别易用的 [mock 功能](#mock)。命令行体验和 create-react-app 一致，配置略有不同，比如默认开启 [css modules](https://github.com/css-modules/css-modules)，**然后还提供了 [JSON 格式的配置方式](https://github.com/sorrycc/dihog#配置)**。

* [介绍 dihog —— 让 create-react-app 可配的命令行工具](https://github.com/sorrycc/blog/issues/15)
* [从 atool-build + dora 到 dihog](https://github.com/sorrycc/blog/issues/17)
...
由于 [create-react-app](https://github.com/facebookincubator/create-react-app) 的默认配置不能满足需求，而他又不提供定制的功能，于是基于他实现了一个可配置版。所以如果既要 create-react-app 的优雅体验，又想定制配置，那么可以试试 dihog 。
...
感谢 create-react-app，dihog 在错误处理上有着良好的体验。此外，dihog 针对 `dihog.json` 的解析错误也做了优化。
...
dihog server 支持 mock 功能，类似 [dora-plugin-proxy](https://github.com/dora-js/dora-plugin-proxy)，在 `dihog.mock.json` 中进行配置，支持基于 require 动态分析的实时刷新，支持 ES6 语法，以及友好的出错提示。
...
配置文件修改的修改会触发 dihog server 的自动重启，会触发重启的文件有：
...
* 格式为 `JSON`，允许注释
* 布尔类型的配置项默认值均为 `false`
* 支持通过 `webpack.config.js` 以编码的方式进行配置，但不推荐，因为 dihog 本身的 major 或 minor 升级可能会引起兼容问题。使用时会给予警告⚠️⚠️⚠️，详见 [#36](https://github.com/sorrycc/dihog/issues/36) 。（`webpack.config.js` 本身的编写支持 ES6，会通过 babal-register 做一层转换。）
...
查看更多[配置相关问题和改进](https://github.com/sorrycc/dihog/issues?q=is%3Aissue+is%3Aopen+label%3Aconfig)。
...
注意：由于 postcss 的插件是以函数的方式进行配置的，所以这个配置只能在 `dihog.json.js` 里使用。
...
Notice:
  - dihog 版本必须 >= `0.6.0-beta1`。
  - 因为 `dihog.json` 配置文件优先级大于 `dihog.json.js`, 请先删除 `dihog.json`。

配置一个路径数组, 该路径下的 svg 文件会全部交给 [svg-sprite-loader](https://github.com/kisenka/svg-sprite-loader) 处理

比如，使用 antd-mobile@1 的 [自定义 svg icon](https://mobile.ant.design/components/icon) 功能的用户，可以在 `dihog.json.js` 文件中做如下配置
...
```bash
# OS X, Linux
$ PORT=3000 dihog server

# Windows (cmd.exe)
$ set PORT=3000&&dihog server

# Or use cross-env for all platforms
$ cross-env PORT=3000 dihog server
```
...
```bash
$ dihog test -h
Usage: dihog test [options] [mocha-options]
...
首先是 JSON 的方式比较简单，`true`/`false` 或是一些简单的字符串就可完成配置；另外，JSON 方式能有效控制使用场景，而编程式的非常不可控，dihog 的一个简单改动都可能导致之前的配置不可用。

### 为什么叫 dihog ?

dihog 即路霸，和 [dva](https://github.com/dvajs/dva) 一样，是守望先锋中的另一名英雄，希望能为 dva 保驾护航。
...
```bash
# Mac, Ubuntu
$ BROWSER=none dihog server

# Windows
$ set BROWSER=none&&dihog server

# Or use cross-env for all platforms
$ cross-env BROWSER=none dihog server
```
