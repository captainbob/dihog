...
export default function (args, appBuild, config, paths) {
  ...
  const dllPlugins = config.dllPlugin ? [
    ...
    new CopyWebpackPlugin([
      {
        from: join(paths.dllNodeModule, 'dihog.dll.js'),
        to: join(paths.appBuild, 'dihog.dll.js'),
      },
    ]),
  ] : [];
  ...
  const finalWebpackConfig = {
    ...
    plugins: [
      new ExportsPlugin({ appSrc: paths.appSrc, appNodeModules: paths.appNodeModules }),
      ...dllPlugins,
      ...(watch ? [] : [
